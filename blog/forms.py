...
class FormCriarConta(FlaskForm):
    ...
    def validate_username(self, username):
        ...
        if usuario:
            raise ValidationError('Usuario já cadastrado. Escolha outro nome de usuario.')
...
class FormEditarPerfil(FlaskForm):
    username = StringField('Nome de usuário', validators=[DataRequired()])
    email = StringField('E-mail', validators=[DataRequired(), Email()])
    foto_perfil = FileField('Atualizar Foto', validators=[FileAllowed(['jpg', 'jpeg', 'png'])])
    ...
    def validate_email(self, email):
        if current_user.email != email.data:
            usuario = Usuario.query.filter_by(email=email.data).first()
            if usuario:
                raise ValidationError('Já existe um usuário com esse email. Utilize outro email.')
